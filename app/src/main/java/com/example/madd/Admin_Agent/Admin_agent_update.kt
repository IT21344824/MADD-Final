...
class Admin_agent_update : AppCompatActivity() {
    private lateinit var binding: ActivityAdminAgentUpdateBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityAdminAgentUpdateBinding.inflate(layoutInflater)
        ...
        binding.AgentAdminUpdateButton.setOnClickListener {
            // Disable the button
            binding.AgentAdminUpdateButton.isEnabled = false

            val name = binding.AdminAgentUpdateName.text.toString().trim()
            val email = binding.AdminAgentUpdateEmail.text.toString().trim()
            val phone = binding.AdminAgentUpdatePhone.text.toString().trim()
            val descrip = binding.AdminAgentUpdateDescip.text.toString().trim()
            ...
            if (name.isEmpty() || email.isEmpty() || phone.isEmpty() || descrip.isEmpty() || img == null) {
                ...
                // Enable the button
                binding.AgentAdminUpdateButton.isEnabled = true
            ...
            Toast.makeText(this, "UpDating Data. Please wait...", Toast.LENGTH_LONG).show()
...
//                            finish()
            ...
            databaseRef.updateChildren(updatedValues).addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    Toast.makeText(this, "Agent updated successfully", Toast.LENGTH_SHORT).show()
                    finish()
                } else {
